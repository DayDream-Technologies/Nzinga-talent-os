          ...
          {tab==="Details"&&<div>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10 }}>
              ...
              <Section title="Social Profile" accent={T.cyan}>
                {[["Handle",local.social_handle||local.stage_name||"—"],["Stage Name",local.stage_name||"—"],["Followers",local.follower_count||"—"],["ER%",local.er_pct?local.er_pct+"%":"—"],["Platform",local.platform||"—"]].map(([k,v])=><div key={k} style={{ display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid #f5f5f5",fontSize:12 }}><span style={{ color:T.t3 }}>{k}</span><span style={{ color:T.t1 }}>{v}</span></div>)}
                {role==="scout"&&local.stage==="holding_entry"&&!scoutReadOnly&&<div style={{ marginTop:6 }}><Lbl>Niches</Lbl><div style={{ display:"flex",gap:5,flexWrap:"wrap" }}>{["Model","Actor","Influencer","Athlete"].map(n=><label key={n} style={{ display:"flex",alignItems:"center",gap:3,fontSize:11,cursor:"pointer",color:local.niches.includes(n)?T.purple:T.t3 }}><input type="checkbox" checked={local.niches.includes(n)} onChange={e=>p("niches",e.target.checked?[...local.niches,n]:local.niches.filter(x=>x!==n))}/>{n}</label>)}</div></div>}
              </Section>
              <Section title="Revenue" accent={T.green}>
                {[["Rep",local.rep_type||"—"],["Commission",local.commission?local.commission+"%":"—"],["Day Rate",local.min_day_rate||"—"],["YTD","$"+parseInt(local.revenue_ytd||0).toLocaleString()],["Projected","$"+parseInt(local.revenue_projected||0).toLocaleString()]].map(([k,v])=><div key={k} style={{ display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid #f5f5f5",fontSize:12 }}><span style={{ color:T.t3 }}>{k}</span><span style={{ color:T.t1,fontWeight:500 }}>{v}</span></div>)}
              </Section>
            </div>
            {(local.roster_division||local.union_affiliation||local.height||local.earliest_availability||local.scout_notes)&&<div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10 }}>
              <Section title="Roster & Casting" accent={T.purple}>
                {[["Division",local.roster_division||"—"],["Specialization",local.secondary_specialization||"—"],["Available",local.earliest_availability||"—"],["Contract Pref",local.contract_duration_pref||local.term_length||"—"],["Travel",local.travel_logistics||"—"]].map(([k,v])=><div key={k} style={{ display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid #f5f5f5",fontSize:12 }}><span style={{ color:T.t3 }}>{k}</span><span style={{ color:T.t1,fontWeight:500 }}>{v}</span></div>)}
              </Section>
              <Section title="Background & Skills" accent={T.amber}>
                {[["Union",local.union_affiliation||"—"],["Guardian Req'd",local.parent_guardian_required||local.legal_minor_status||"—"],["Ref Check",local.reference_check_status||"—"],["Other Agency",local.current_agency||"—"],["Prior Revenue",local.prior_annual_revenue||"—"]].map(([k,v])=><div key={k} style={{ display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid #f5f5f5",fontSize:12 }}><span style={{ color:T.t3 }}>{k}</span><span style={{ color:T.t1,fontWeight:500 }}>{v}</span></div>)}
              </Section>
              <Section title="Physical Specs" accent={T.cyan}>
                {[["Height",local.height||"—"],["Bust/Waist/Hips",[local.bust,local.waist,local.hips].filter(Boolean).join(" / ")||"—"],["Shoe",local.shoe_size||"—"],["Eyes",local.eye_color||"—"],["Gov ID",local.gov_id_number||"—"]].map(([k,v])=><div key={k} style={{ display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid #f5f5f5",fontSize:12 }}><span style={{ color:T.t3 }}>{k}</span><span style={{ color:T.t1,fontWeight:500 }}>{v}</span></div>)}
              </Section>
            </div>}
            {local.scout_notes&&<Section title="Scout Notes & Audition Feedback" accent={T.purple} style={{ marginBottom:10 }}>
              <p style={{ fontSize:12,color:T.t2,margin:0,whiteSpace:"pre-wrap" }}>{local.scout_notes}</p>
            </Section>}
