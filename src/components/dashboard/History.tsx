...
function HistoryMod({ history, setHistory, talents, currentUser, initialFilter }) {
  ...
  return (
    <div style={{ padding: '14px 18px', flex: 1, overflowY: 'auto' }}>
      ...
      <div style={{ background: T.cardBg, border: `1px solid ${T.cardBorder}`, borderRadius: 8, overflow: 'hidden' }}>
        {filt.length === 0 ? (
...
function Reports({ talents, userRole, applications, initialTab }) {
  const [active,setActive]=useState(initialTab || "pipeline_summary");
  useEffect(() => {
  ...
  const apps=applications ? Object.values(applications) : [];
  const rpts=[{id:"pipeline_summary",label:"Pipeline Summary"},{id:"jordan_scores",label:"Jordan Score Report"},{id:"revenue_forecast",label:"Revenue Forecast"},{id:"compliance_status",label:"Compliance Status"},{id:"prospect_box_score",label:"Prospect Box Score"}];
  return <div style={{ padding:"14px 18px",flex:1,overflowY:"auto",display:"flex",gap:14 }}>
    <div style={{ width:170,flexShrink:0 }}>
      <div style={{ background: T.cardBg,border:`1px solid ${T.cardBorder}`,borderRadius:8,overflow:"hidden" }}>
        <div style={{ padding:"8px 10px",borderBottom:"2px solid "+T.blue,background:T.mutedBg,fontSize:11,fontWeight:700,color:T.t1,textTransform:"uppercase",letterSpacing:"0.06em" }}>Reports</div>
        {rpts.map(r=><div key={r.id} onClick={()=>setActive(r.id)} style={{ padding:"7px 10px",cursor:"pointer",fontSize:12,color:active===r.id?T.blue:T.t2,background:active===r.id?T.blueL:"transparent",borderLeft:`3px solid ${active===r.id?T.blue:"transparent"}`,fontWeight:active===r.id?600:400 }}>{r.label}</div>)}
      </div>
    </div>
    <div style={{ flex:1 }}>
      <div style={{ background: T.cardBg,border:`1px solid ${T.cardBorder}`,borderRadius:8,overflow:"hidden" }}>
        {active==="pipeline_summary"&&<>
          <div style={{ padding:"9px 12px",borderBottom:"2px solid "+T.blue,background:T.mutedBg }}><span style={{ fontSize:12,fontWeight:700,color:T.t1,textTransform:"uppercase" }}>Pipeline Summary</span></div>
        ...
        {active==="jordan_scores"&&<>
          <div style={{ padding:"9px 12px",borderBottom:"2px solid "+T.purple,background:T.mutedBg }}><span style={{ fontSize:12,fontWeight:700,color:T.t1,textTransform:"uppercase" }}>Jordan Score Report</span></div>
        ...
        {active==="revenue_forecast"&&<>
          <div style={{ padding:"9px 12px",borderBottom:"2px solid "+T.green,background:T.mutedBg }}><span style={{ fontSize:12,fontWeight:700,color:T.t1,textTransform:"uppercase" }}>Revenue Forecast</span></div>
        ...
        {active==="compliance_status"&&<>
          <div style={{ padding:"9px 12px",borderBottom:"2px solid "+T.blue,background:T.mutedBg }}><span style={{ fontSize:12,fontWeight:700,color:T.t1,textTransform:"uppercase" }}>Compliance Status</span></div>
        ...
        {active==="prospect_box_score"&&<>
          <div style={{ padding:"9px 12px",borderBottom:"2px solid "+T.purple,background:T.mutedBg }}><span style={{ fontSize:12,fontWeight:700,color:T.t1,textTransform:"uppercase" }}>Prospect Box Score</span></div>
          <table style={{ width:"100%",borderCollapse:"collapse" }}><thead><tr><TH>Prospect</TH><TH>Status</TH><TH>Progress</TH><TH>Jordan Score</TH><TH>Stage</TH><TH>Imported</TH></tr></thead><tbody>
            {apps.length===0?<tr><TD colSpan={6} muted>No applications yet</TD></tr>:apps.sort((a,b)=>new Date(b.last_saved||b.created_at)-new Date(a.last_saved||a.created_at)).map(app=>{
